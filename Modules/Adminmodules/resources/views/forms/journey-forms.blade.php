        ...
        <div class="sm:ml-64">
            ...
            <main class="px-4 sm:px-6 py-6">
                <div class="bg-white rounded-2xl border border-slate-200 p-6">
                    <div class="text-sm text-slate-500">Journey Stage</div>
                    <h1 class="mt-1 text-2xl font-extrabold text-slate-900">{{ $pageTitle ?? 'Forms' }}</h1>
                    <p class="mt-2 text-slate-600">Hapa tunaonyesha waliopo kwenye stage hii tu. Unaweza ku-view details na ku-approve.</p>
                    @if(!empty($defaultJourneyStage))
                        <div class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 text-sm font-extrabold">
                            ...
                            <a href="{{ url('/admin/forms') }}" class="ml-2 text-slate-600 hover:text-slate-900 font-semibold">All Forms</a>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="px-6 py-4 border-b border-slate-200 bg-white">
                        <div class="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                <button id="btn-refresh" class="relative px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-800 font-semibold transition">
            ...
            (function () {
                ...
                async function fetchList() {
                    ...
                    try {
                        ...
                        const json = await res.json().catch(function () { return null; });
                        if (!res.ok) {
                            throw new Error((json && json.message) ? json.message : 'Imeshindikana kupata data.');
                        }
