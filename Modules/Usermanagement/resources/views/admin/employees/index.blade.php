<x-adminmodules::layouts.master>
    <div class="min-h-screen bg-slate-50">
                ...
                <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden">
                        ...
                        <div class="flex items-center gap-3">
                            <div>
                                <div class="flex items-center gap-2">
                                    <h1 class="text-base font-extrabold text-slate-900">Wafanyakazi</h1>
                                    <span class="text-sm text-slate-500">{{ number_format($employees->total()) }} Matokeo</span>
                                    <span class="material-symbols-outlined text-slate-400" style="font-size: 18px">info</span>
                                </div>
                                <div class="mt-1 text-xs text-slate-500">Usimamizi wa Wafanyakazi</div>
                            </div>
                        </div>

                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.employees.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-600 text-white font-semibold transition">
                                Sajili Mfanyakazi
                            </a>
                    ...
                    <div class="px-6 py-4 border-t border-slate-200 bg-white flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div class="flex items-center gap-3 flex-wrap">
                            ...
                            <button type="button" class="px-3 py-1.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-800 hover:bg-slate-50">Position</button>
                            <button type="button" class="px-3 py-1.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-800 hover:bg-slate-50">Hired date</button>
                        ...
                        <form method="get" class="flex items-center gap-2">
                            <div class="relative">
                                ...
                                <input name="q" value="{{ $q ?? '' }}" placeholder="Tafuta mfanyakazi" class="pl-10 pr-4 py-2.5 w-72 max-w-full rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-200 focus:border-emerald-300" />
                            </div>
                        ...
                        <table class="min-w-full text-sm">
                            <thead class="text-xs uppercase text-slate-500 bg-slate-50 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="text-left py-3 px-4">Mfanyakazi</th>
                                    <th class="text-left py-3 px-4">Kodi</th>
                                    <th class="text-left py-3 px-4">Barua pepe</th>
                                    <th class="text-left py-3 px-4">Simu</th>
                                    <th class="text-left py-3 px-4">Hali</th>
                                    <th class="text-left py-3 px-4">Ameajiriwa</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse($employees as $employee)
                                    <tr class="hover:bg-slate-50">
                                        ...
                                        <td class="py-3 px-4">
                                            ...
                                            <div class="text-xs text-slate-500">Mtumiaji: {{ $employee->user?->email ?? '-' }}</div>
                                        </td>
                                        ...
                                        <td class="py-3 px-4">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.employees.show', $employee) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 transition" title="Maelezo">
                                                ...
                                                <button type="button" class="inline-flex items-center justify-center w-9 h-9 rounded-xl border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 transition" title="Futa" data-delete-employee-button data-delete-employee-id="{{ $employee->id }}" data-delete-employee-name="{{ $employee->full_name }}" data-delete-employee-action="{{ route('admin.employees.destroy', $employee) }}">
                                                ...
                                                <a href="{{ route('admin.employees.logs', $employee) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 transition" title="Rekodi">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="py-8 px-4 text-slate-500">Hakuna wafanyakazi.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div id="employee-info-popup" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-lg h-full md:h-auto">
                <div class="relative p-4 bg-white rounded-2xl border border-slate-200 shadow md:p-8">
                    <div class="mb-4 text-sm font-light text-slate-600">
                        ...
                        <p>
                            Unakaribia kumfuta <span class="font-extrabold text-slate-900" data-delete-employee-name></span>.
                    ...
                    <div class="justify-between items-center pt-0 space-y-4 sm:flex sm:space-y-0">
                        ...
                        <div class="items-center space-y-4 sm:space-x-4 sm:flex sm:space-y-0">
                            <button id="employee-close-modal" type="button" class="py-2.5 px-4 w-full text-sm font-semibold text-slate-600 bg-white rounded-xl border border-slate-200 sm:w-auto hover:bg-slate-50 focus:ring-4 focus:outline-none focus:ring-primary-200 hover:text-slate-900 focus:z-10">Ghairi</button>
                            <form method="POST" id="delete-employee-form" action="#" class="w-full sm:w-auto">
                                @csrf
                                @method('DELETE')
                                <button id="employee-confirm-button" type="submit" class="py-2.5 px-4 w-full text-sm font-semibold text-center text-white rounded-xl bg-rose-600 sm:w-auto hover:bg-rose-500 focus:ring-4 focus:outline-none focus:ring-rose-200">Thibitisha</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <script>
            (function () {
                const modalEl = document.getElementById('employee-info-popup');
                ...
                const nameEl = modalEl.querySelector('[data-delete-employee-name]');
                const formEl = document.getElementById('delete-employee-form');
                ...
                document.addEventListener('click', function (e) {
                    const btn = e.target.closest && e.target.closest('[data-delete-employee-button]');
                    if (btn) {
                        openModal(btn.getAttribute('data-delete-employee-name'), btn.getAttribute('data-delete-employee-action'));
                        return;
                    }
                });

                const closeBtn = document.getElementById('employee-close-modal');
